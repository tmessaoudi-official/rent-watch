...
| `rm -f seen.json` / deleting the listings DB | Wipes the seen-set, so the **next run re-notifies every listing it can still see** — a self-inflicted notification flood, and price history is gone for good | Whenever the store holds anything; the flood is the point of failure, not the file loss |
