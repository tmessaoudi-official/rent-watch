...
implementation of `models` + `tenure` under `src/php/Core/`, a 93-case language-neutral classifier
...
- **Classifier tests.** ≥30 hand-labelled listing texts covering pure-LLI In'li, mixed CDC Habitat,
  ...
  regresses. **Done** — `tests/fixtures/tenure/corpus.json`, 93 cases, and the suite asserts all five
  shapes are present so "30 easy ones" cannot satisfy it. The corpus is **still 93/93 synthetic**:
