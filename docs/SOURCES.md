...
| **B3** | **PAP** | `www.pap.fr` | **LIVE 2026-08-26** | ✅ **Source #8, and the first DIRECT-FROM-OWNER portal** — *particulier à particulier*, no agency fees, so its inventory does not overlap the agency portals every other private source draws from. `scout doctor --source=pap`: **2 annonces, `ok`, 483 ms**. Structurally the simplest alert in the tree: a real `text/plain` part, ONE listing per message (no `card_separator`), and a real ad id (`/annonces/-r458301723`), so identity is the link. **What it cost was two defects a naive config walks into, both MEASURED before any config was written.** (1) The alert quotes the SUBSCRIBER'S OWN search criteria above the listing — *"jusqu'à 1.200 EUR à partir de 45 m²"* — and every generic reader is first-match-wins, so the surface read **45**, the search FLOOR, instead of the flat's 50: below `min_surface_m2`, so the first alert ever sent would have been rejected as too small, silently. Bien'ici's defect a second time, down to the same number, because the same saved search is used on every portal. Fixed with four POSITIONAL anchors on the `(NNNNN)` postcode line — the numeric twin of *a title is a position, never a vocabulary*. (2) `link_host` must carry the PATH: the message has two links and BOTH are on `www.pap.fr`, and the unsubscribe page at `/utilisateur/alertes` matches none of the noise words `looksLikeAListing()` rejects, so a host-only value yielded a phantom second listing carrying the real flat's facts under its own identity — never delisted. Two real alerts frozen at `tests/fixtures/pap/`; offline proof is `MAILBOX_DIR=tests/fixtures/pap`. See `docs/plans/pap-email-alert.plan.md`. |
