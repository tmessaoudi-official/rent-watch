...
| F22 | One re-scrubbed ParuVendu fixture is no longer byte-what-the-mailer-emitted | The hex replacer's fold-aware pattern matched ACROSS a quoted-printable soft break and its replacement was shorter than the fold width, so two lines were joined into one **152-column** line (RFC 2045 allows 76) and `=F0` became `=f0`. It parses and every test passes; the decoded payload is otherwise byte-identical. Not hand-edited, because editing a fixture by hand is what the doctrine warns against — the general fix is for `$refold` to preserve the original's line structure when the replacement is shorter than the fold | **OPEN — round 5, P3** |
