...
- [2026-08-29 00:10] VERIFIED LIVE, against the real mailbox on a THROWAWAY `SCOUT_DB` (never the production one — see the redeploy trap above): `scout doctor --source=leboncoin` returns **`ok`, 3 annonces, 952 ms, schéma v11**, and `SELECT feed_newest_at FROM source_runs` returns **`2026-08-26T05:33:06Z`**. That is the real `Date: Wed, 26 Aug 2026 07:33:06 +0200` header, normalised to UTC, through the whole chain — `ImapMailbox` → `EmailAlertSource` → `Pipeline`/`doctor` → the new column. **No offline test exercises that chain**, which is why the poll was worth one request.
- [2026-08-29 00:10] MEASURED, and it is a DATED PREDICTION rather than a claim: the message is **64.6 h old against the 72 h threshold**, so `ok` is the correct verdict today. It flips to **`feed_silent` at about 09:33 Paris on 2026-08-29** — roughly seven hours from this entry, and **four days before** the 2 September window expiry that would otherwise have reported the same silence as `BROKEN`, on the wrong day and with the wrong cause. If it has NOT flipped by then, either the alert fired again (the good outcome, and the count will have moved) or something in this chain is wrong; there is no third reading.
- [2026-08-29 08:30] PANEL ROUND 1 (`8c5d332`, MAXIMAL, three unnamed lenses in a pinned worktree): **20 findings, ~13 distinct.** The panel earned its keep decisively and most of the serious findings were mine. Fixed in `f58ac14`'s successor; every claim below is the reviewers' own execution evidence, not narrative.
- [2026-08-29 08:30] FIXED, P1 — **the entire production path that PRODUCES a feed date was unpinned, and all three lenses independently cut it with the suite green.** `EmailAlertSource::newestFeedItemAt() { return null; }` was the MASTER SWITCH: one line, no test failure, and `FEED_SILENT` dead in production for `seloger`, `bienici`, `leboncoin` and `pap` at once. So were `ImapMailbox::noteMessageDate()` as a no-op, `Pipeline` not reading it, `doctor` not recording it, and `FileMailbox` returning a real date. The store side was pinned and the source side was not, so the verdict could be judged perfectly on rows nothing would ever write. The excuse — *"no offline test can reach it"* — was FALSE: the method is pure given a raw message, and this repo already reflects into that class's privates. `tests/php/Adapters/FeedFreshnessTest.php` (11 tests) plus ten new ledger cases.
- [2026-08-29 08:30] FIXED, P1 — **`new \DateTimeImmutable($header)` is a RELATIVE-EXPRESSION parser and misreads dates FORWARD.** `Date: Fri, 09 Aug 2026 08:00:00 +0200` — 9 August is a Sunday — applies `Fri` as a modifier and records **14 August**, five days on; `now`, `tomorrow` and `+2 days` all parse as literal dates. Since the observable band is `IMAP_SINCE_DAYS − FEED_SILENT_DAYS` = four days, a +5 shift ages the message out of the search window — collapsing the count the verdict gates on — before the shifted date can reach the threshold. **One bad header suppresses the verdict on that source for ever**, and the `catch (\Exception)` guarded a case that mostly does not happen. Replaced with `parseRfc2822()`, strict **by round-trip**, the technique `Store::epoch()` already carries a scar for. `createFromFormat(RFC2822)` alone is NOT sufficient — measured, it also yields 14 August and reports no error.
- [2026-08-29 08:30] REVERSED (developer ruling, `AskUserQuestion`) — **the `FEED_SILENT_DAYS >= IMAP_SINCE_DAYS` startup refusal is DEMOTED to a `doctor` warning, because both of its legs broke.** Its premise was *"the newest message `SEARCH SINCE` can match is by definition at most `IMAP_SINCE_DAYS` old"*. **That is false: `SEARCH SINCE` filters on INTERNALDATE — server arrival — while the threshold is measured against the message's own `Date:` header.** A message delivered today and stamped weeks ago (bulk re-label, delayed relay — the 2026-08-25 incident shape) is inside the window and arbitrarily old; a reviewer demonstrated `feed_silent … depuis 20 jour(s)`. And it **locked the tool out**: `IMAP_SINCE_DAYS=1`, which `.env.example` documents as meaningful and `ImapMailbox` clamps to happily, left no integer satisfying `1 <= d < 1`, so `doctor`, `dump`, `run`, `digest` and `reclassify` all exited 2 — *including on deployments with no email source at all*, a regression on a value previously just clamped — while a refused `run` wrote `last-refusal.txt` to be reported on the next successful start, which could never come. Under Docker that is a crash loop writing a note nobody reads, verbatim the reader Q27 exists for. **`doctor` DIAGNOSES, it does not refuse** — the same shape already applied to an unusable `TZ`. The `< 1` refusal is sound and stays.
- [2026-08-29 08:30] FIXED, P2 — **`max()` over feed dates was a LEXICAL comparison** while the store accepts any RFC 3339 offset (its own CLI test writes `+02:00`). Measured on the real store: a feed **one hour old** was reported as **one day silent**, naming the wrong message. One-directional — the lexical max is never a later instant than the true max — so it could only ever OVER-state silence, which is why it was P2 and not P0. Both the verdict and the `BROKEN` detail now read through one helper that compares instants, because a detail naming a date the verdict refused to believe is a diagnostic contradicting its own conclusion.
- [2026-08-29 08:30] FIXED, P2 — **`ImapMailbox` never reset its cached date on either early-return path**, so a pass that fetched NOTHING reported the previous pass's date as its own — exactly what `Pipeline` documents must never happen, while guarding only the exception path. Harmless *only* because `Store::health()` gates on `$lastCount > 0`, in a different file, individually deletable. **One mechanism accidentally protected by another**, which is this repo's own named trap: never read *"no harm occurred"* as *"the rule held"*. The reset moved to the top of the fetch.
- [2026-08-29 08:30] FIXED, P3 — **a healthy source's FIRST pass could report *"vérifiez l'horloge du portail"*.** `Pipeline::runOnce()` captures one `$nowIso` at pass start and Q37 pacing puts 5 s between hosts and 60 s per host, so a source polled minutes later legitimately sees a message stamped after that instant; on a fresh database that single row is the whole history, so no date is credible. A one-hour grace covers any pass and sits far below the smallest threshold (one day), so it cannot mask real silence — and `scout run --seed` on a new database is exactly when someone is watching.
- [2026-08-29 08:30] CORRECTED — **two claims I wrote were false and are now the opposite.** `Scout.php` said *"the per-source override exists for exactly that"*: there is no such key, and adding one to a source block is a `ConfigError`, because unknown source keys are refused. `FileMailbox::newestMessageAt()` said *"Deliberate, and **asserted**"*: no test called it. It is asserted now, and that assertion is load-bearing today rather than hypothetically — the fixtures are dated 25–26 August against a three-day default, so the moment it returns a real date the documented `MAILBOX_DIR=… scout doctor` workflow reports `feed_silent`, a gate that reddens with the calendar and no code change.
- [2026-08-29 08:30] DECIDED — **`FEED_SILENT` stays `Priority::LOW`, and that is now a decision rather than a fall-through.** Two lenses flagged that it entered the `else` branch with nobody choosing, beside a comment justifying `NORMAL` for `BROKEN` on the grounds of being *"indistinguishable from a quiet market"* — which is the new status's own definition. The argument that wins is twin-consistency: `SourceStatus` calls `STALE` *"FEED_SILENT's twin from the other end"*, `STALE` is LOW, `NEVER_PRODUCED` is LOW, and splitting the twins would assert that the portal stopping matters more than the watcher stopping. Revisit together, never separately.
- [2026-08-29 08:30] NOTED — **the apply-sweep caught four of the new ledger expressions matching NOTHING**, and it was right each time: two used multi-line `\n` patterns, which `sed` cannot match line-by-line; one targeted the credible filter after it moved into a helper; and one targeted the refusal this round deleted. Retargeted, not removed, per the sweep's own instruction. `all 527 expressions still apply`.
- [2026-08-29 09:40] FIXED, found by the ledger and by nothing else — **the pipeline's WRITE of the feed date was undetected.** `FeedFreshnessTest` proves the source hands a date to the store when called directly, and `StoreFeedSilenceTest` proves the store judges such rows correctly, but nothing proved the PIPELINE is what carries one to the other: replacing that line with `null` left the suite green. That is the sixth link of the same chain the panel found five cuts in. Pinned at COLUMN level in `PipelineRunTest` — asserting the verdict instead would pass on a row whose date came from anywhere, and it is the write that was deletable.
- [2026-08-29 09:40] NOTED, and it is the better answer than a second test — **`doctor` had the same untested write, and it CANNOT be tested.** Observing it through the CLI needs an `email_alert` source whose mailbox reports a date; the only offline mailbox is `FileMailbox`, which returns `null` on purpose and must keep doing so. **A test that cannot exist is not a reason to leave code untested — it is a reason for the code not to be DUPLICATED.** The two `instanceof FeedFreshness ? … : null` sites were collapsed onto `Adapters/FeedDate::of()`, so the pipeline's coverage now covers both writers and the untested surface went from two sites to zero. The ledger case follows the code: it targets the shared reader.
- [2026-08-29 10:30] SABOTAGE ROUND on the feed-silence set, frozen tree: **15 detected, 2 undetected** — and **both undetected cases were WEAK TESTS, not weak code.** That distinction is the whole value of running it: the suite was green, the guarantees were implemented correctly, and two of them would not have been NOTICED breaking.
  1. **The lexical-vs-instant test could not discriminate.** Its two dates were three hours apart against a one-day threshold, so the correct answer and the broken one were BOTH `OK`. The offsets have to be pushed to the ends of the range — `-12:00` and `+14:00`, 26 hours apart — so the lexical pick lands on the far side of the threshold from the true one. **A test whose two branches produce the same answer asserts nothing**, and it passes exactly as convincingly as a real one.
  2. **The per-fetch reset had no test at all.** `noteMessageDate()` was pinned by reflection; `fetchRecent()`'s reset was not. It IS testable offline, and for a precise reason: the reset is the FIRST statement, ahead of `connect()`, so the offline tripwire throws a moment later and the guarantee has already been observed. The "needs a socket" excuse was wrong here in the same way it was wrong for the parser.
- [2026-08-29 10:30] VERIFIED BY HAND, because a green suite proves nothing about either fix: each mutation was applied to a THROWAWAY copy using the ledger's own `sed` expression, checked to be valid PHP, and confirmed to turn the suite red — `Failures: 1` in both cases. The live tree was never mutated; `git status` after the probes shows only the two test files.
