...
| `tests/fixtures/tenure/corpus.json` | 105 hand-labelled listing texts — the classifier's ground truth, and language-neutral so the phorj port reads the same file. **All 105 are synthetic**: `spec/PROJECT_BRIEF.md` §4 asks for *real* texts, and capturing those needs a source endpoint that does not exist yet. Every case declares its `provenance` and a test asserts the counts, so the gap is data rather than a promise |
...
2. **Tenure classifier + tests.** ✅ **Done in PHP**, against a 105-case synthetic corpus — spec §4's
