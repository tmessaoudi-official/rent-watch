 ...
 * {@see RentScout::buildSource()} passed `null`. So robots.txt was enforced in tests, by injection, and
 ...
 *
 * These tests therefore refuse to go anywhere near adapter injection. They drive the real `RentScout`,
 * against a real `config/rent/sources.json`, and assert on what the CLI prints. The failing-first shape
 ...
 */
#[CoversNothing]
final class RentScoutRobotsTest extends TestCase
{
    /** @var list<string> */
    ...
    private function doctor(ScriptedHttpClient $client, ?array $sources = null, ?string $only = 'robots_probe'): array
    {
        ...
        $code = (new RentScout($root, $out, $err, '2026-08-07T12:00:00+02:00', $client))
    ...
    private function tempRoot(?array $sources = null): string
    {
        ...
        file_put_contents($root . '/config/rent/criteria.json', json_encode([
