...
|---|---|
| A file/path layout claim | `ls` / `find` the path. A documented path that does not exist is STALE — and right now `src/`, `config/` and `tests/` are all documented-as-target and absent, which `CLAUDE.md` states explicitly. Report a path claim as stale only where the doc asserts it EXISTS. |
| "the classifier corpus has ≥30 hand-labelled cases" | `ls tests/fixtures/tenure/ \| wc -l` — the brief's §4 number is a requirement; the count is the check |
