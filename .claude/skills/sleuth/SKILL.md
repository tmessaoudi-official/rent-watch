...
Use `--focus=<A|B|C|D|E|F|G|H|I|J|K>` (single agent), `--target=<path>` (analyze a specific directory), `--quick` (A+B+F only — logic, silent failures, shell bugs). **`--scope=global|both` is REMOVED here** (adaptation): `~/.claude/` is the developer's own persistent install, out of this repo's audit scope — audit it from its own sessions, not from here. Project scope only.
